...
def main():
    ...
    while True:
        ...
        # Change the user-agent header for each request
        user_agent = random.choice(regular_headers)
        s.send("User-agent: {}".format(user_agent).encode('UTF-8'))

        # Add a random amount of data to the request body
        data_size = random.randint(1, 1024)
        s.send(b'x' * data_size)

        # Add a random amount of noise to the request headers
        for header in regular_headers:
